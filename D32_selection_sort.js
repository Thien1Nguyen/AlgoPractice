...
function selectionSort(nums = []) {
    ...
    function selectionSort(numbers = []) {
        ...
        for (let i = 0; i < numbers.length; i++) { // outer loop (i loop) keeps track of where we are
            let min = numbers[i] // create a min var, and a min idx var, current
            let minIdx = i

            for (let j = i; j < numbers.length; j++) { // inner loop (j loop) keeps track of minimum
                if (numbers[j] < min) { // inside j loop have a conditional that compares for min
                    min = numbers[j] // inside the if, we have found a smaller val, so make that the new min
                    minIdx = j
                }
            }
            if (numbers[i] != numbers[minIdx]) { // after j loop swap (if current and min are different)
